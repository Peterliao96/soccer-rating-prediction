...
rt$cptable[,c(2:5,1)]
rt.cpt <- rt$cptable
minrow <- which.min(rt.cpt[,4])
se.row <- min(which(rt.cpt[,4] < rt.cpt[minrow,4] + rt.cpt[minrow,5]))
cplow.1se <- rt.cpt[se.row,1]
cpup.1se <- ifelse(se.row==1,yes=1, no = rt.cpt[se.row-1,1])
cp.1se <- sqrt(cplow.1se*cpup.1se)
prune.1se <- prune(rt,cp = cp.1se)
plotcp(prune.1se)

cplow.min <- rt.cpt[minrow,1]
cpup.min <- ifelse(minrow==1, yes = 1, no = rt.cpt[minrow-1,1])
cp.min <- sqrt(cplow.min*cpup.min)
prune.mincp <- prune(rt,cp = cp.min)
plotcp(prune.mincp)
prp(prune.1se, type=1, extra=1, main="Pruned Tree")
prp(prune.mincp, type=1, extra=1, main="Pruned minTree")
